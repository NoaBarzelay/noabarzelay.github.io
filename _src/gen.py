    ...
    <section id="projects" class="block">
      ...
      <div class="cards">
        ...
        <article class="card">
          <div class="card-top">
            <h3>Precept</h3>
            <div class="taglist"><span class="pill">TypeScript</span><span class="pill">Bun</span><span class="pill">Claude Code</span><span class="pill">Local-first</span><span class="pill">Harness</span><span class="pill">Obsidian</span></div>
          </div>
          ...
          <p>It attaches through several surfaces: the pre-tool, prompt-submit, and session-end hooks, the session transcript, and a review CLI. Off the interactive turn, a reader turns each session into evidence, diffing what the agent wrote against the file's final state to catch a silent edit, and an inference pass proposes at most one durable item or abstains behind a cost filter, grounding its confidence in observable signals rather than the model's own say-so. I keep, correct, or dismiss each candidate before anything is written, and only a turn I typed can author an enforcing rule, so content read mid-session cannot plant one. The catalog is versioned markdown cards indexed in SQLite, knowledge is filed into my Obsidian vault, and the must-hold rules compile to a plain check cache the pre-tool hook evaluates in milliseconds with no model or parser on the path. Every rule is judged against the real calls it would have fired on rather than a rationale, a new one stays probationary until it proves itself, and the offline suite and architecture fitness functions run in CI on every push.</p>
